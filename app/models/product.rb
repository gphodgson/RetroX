...
class Product < ApplicationRecord
  ...
  def self.new_products_query
    if Rails.env.development?
      ...
      "DATE_PART('day', current_timestamp-created_at) <= #{NEW_PRODUCT_THRESHOLD}"
    end
  end
  ...
  def self.recently_updated_products_query
    if Rails.env.development?
      ...
      "DATE_PART('day', current_timestamp-updated_at) <= #{RECENT_UPDATE_THRESHOLD} AND NOT (#{new_products_query})"
    end
  end
